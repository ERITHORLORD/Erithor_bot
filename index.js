...
if(cmd === `${prefix}clear`){
   ...
   let ticket_category_id = "890948018084409366"
   let ticket_role_id = "866718819086893056"
   let support_role_id = "807718733853622352"
...
if(cmd === `${prefix}torles-ticket`){
    if(message.member.roles.cache.has(support_role_id) || message.member.hasPermission("ADMINISTRATOR" || "BAN_MEMBERS")){
        ...
        if(args[0] && args[1] && ping_member && ping_channel && ping_member.roles.cache.has(ticket_role_id)) { 
            ...
            message.reply(`Kérlek említs meg egy olyan embert akin van ticket rang! Helyes használat: ${prefix}torol-ticket <@ember> <@csatorna>`)
        }
    } else {
